...
func getValue(nums [][]int) {
	...
	for i:= 1; i<len(dp); i++{
		for j:= 1; j<len(dp[i]); j++{
			if j == len(dp[i])-1{
				dp[i][j] += max(dp[i][j-1]+nums[i][j], dp[i-1][j-1]+nums[i][j])
			}else{
				dp[i][j] += max(dp[i][j-1]+nums[i][j], max(dp[i-1][j-1]+nums[i][j], dp[i-1][j])+nums[i][j])
			}
		}
	}
	fmt.Println(dp)
...
func main() {
	...
	for i:=0; i<n; i++{
		...
		for j:=0; j<len(strNums); j++{
			nums[i][j],_ = strconv.Atoi(strNums[j])
		}
	}
	getValue(nums)
}